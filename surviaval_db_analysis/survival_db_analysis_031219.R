...
ggsurvplot(survfit(OS ~ df$Fish_DEL_17, data = df), pval = T, risk.table = T, xlab = "OS")
ggsurvplot(survfit(PFS ~ df$Fish_DEL_17, data = df), pval = T, risk.table = T, xlab = "PFS")

coxph(OS ~ df$Fish_DEL_17, data = df) %>% summary
coxph(PFS ~ df$Fish_DEL_17, data = df) %>% summary

df$Fish_DEL_17 %>% table
...
coxph(OS ~ df$call50_del_TP53, data = df) %>% summary
coxph(PFS ~ df$call50_del_TP53, data = df) %>% summary
...
coxph(OS ~ df$call40_del_TP53, data = df) %>% summary
coxph(PFS ~ df$call40_del_TP53, data = df) %>% summary
...
coxph(OS ~ df$call30_del_TP53, data = df) %>% summary
coxph(PFS ~ df$call30_del_TP53, data = df) %>% summary
...
coxph(OS ~ df$call20_del_TP53, data = df) %>% summary
coxph(PFS ~ df$call20_del_TP53, data = df) %>% summary
...
#______ double HIT on TP53 _________
...
#______ double HIT with LOH on TP53 _________

df$double_del_LOH <- ifelse(df$mut_del_event ==1 & df$LOH_call == 1, 1,0)
df$double_del_LOH %>% table
